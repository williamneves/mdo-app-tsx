...
export default function BonusForm(props: IBonusFormProps) {
  // ** Props
  const { employee, setBonusStatus, goal, store, sales, editBonus, display } = props;
  ...
  const draftSave = bonusQ.useUpdateAllBonusQuery(queryClient);
  const saveAsDraft = bonusQ.useCreateBonusQuery(queryClient);
  ...
  // * Default Values
  const defaultValues = ():IBonus => editBonus ? editBonus : ({
    inactive: false,
    deleted: false,
    status: "",
    name: "",
  ...
  useEffect(() => {
    ...
    if (editBonus){
      ...
      bonusBrief.bonusStatus = switchChipStatusLabel(bonus.status);
  ...
  // * Handle Submit
  const onSubmit = async (data: IBonus, draft?: any) => {
    // Round to 2 decimal places
    data.bonusAmount = Math.ceil(getBonusAmount(data) * 100) / 100;
    console.log(data);

    if (draft === 'draft') {
      toast.success("Bônus salvo como rascunho");

      data.status = "draft";

      if (data?._id !== undefined) {
        console.log("update")
        return draftSave.mutate(data)
      }
      console.log("update2")

      return saveAsDraft.mutate(data)
    }

    data.status = "posted";

    toast.success("Bônus Lançado!");
  ...
  // * Handle Chip Status Color
  const switchChipStatusColor = (status: string) => {
    switch (status) {
      case "draft":
        return "warning";
      case "posted":
  ...
  // * Handle Chip Status Label
  const switchChipStatusLabel = (status: string) => {
    switch (status) {
      case "draft":
        return "Rascunho";
      case "posted":
              ...
              return (
                <Grid container spacing={5} item xs={12} key={bonusEntry.id}>
                  ...
                  <Grid item xs={12}>
                    {
                      // @ts-ignore
                      bonusFormWatch(`${fieldName}.type`) === "sales" && autoGeneratedBonus && (
                        <Alert
                          severity="info"
                          sx={{ mb: 2 }}
                        >
                  ...
                  <Grid item xs={6} md={3}>
                    <SelectInputController
                      ...
                      // @ts-ignore
                      readOnly={(Boolean(bonusFormWatch(`${fieldName}.type`) === "sales" && Boolean(autoGeneratedBonus)))}
                      selectItems={{
                        items: [
                          {
                            key: "add",
