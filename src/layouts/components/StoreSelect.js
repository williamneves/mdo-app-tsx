...
const StoreSelect = () => {
  ...
  return (
    <Fragment>
      <Button variant='text' aria-haspopup='true' onClick={handleClick} aria-controls='customized-menu'>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
          <StoreMallDirectoryTwoToneIcon />
          <Typography variant='body2' color={'primary.main'} ml={'0.4rem'}>
            {selectedStore ? selectedStore.name : 'Select Store'}
          </Typography>
          {user.stores.length > 1 && <ArrowDropDownTwoToneIcon sx={{marginX: 0}}/>}
        </Box>
      </Button>
      {user.stores.length > 1 && <Menu
        keepMounted
        selected={null}
        elevation={0}
        anchorEl={anchorEl}
      ...
      >
        {
          user.stores.map((store,index) => {
            return (
              <MenuItem key={store._id} onClick={() => {
                handleSelect(store)
