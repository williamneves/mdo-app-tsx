...
const ApproveSales = () => {
  ...
  const [rangeDate, setRangeDate] = useState<GetDateRange>(
  ...
  const handleUpdateDates = () => {
    ...
    setFilteredData(
      filterSalesByDate("customPeriod", {
  ...
  const changeAuditStatus = async (
    ...
    try {
      await changeSaleStatus.mutateAsync({
        saleID,
        status,
  ...
  const approveAllSales = async () => {
    ...
    try {
      await Promise.all(
        (pendingSales as Sale[]).map(async (sale: Sale) => {
          // set a timeout to get maximum 10 operation per second
          setTimeout(
            () =>
              changeSaleStatus.mutate({
                saleID: sale._id,
                status: "approved",
                auditFeedBack: ""
              }),
  ...
  const handleApproveRejectSale = (
    ...
    setDialogData({
      ...
      content: (
        <DialogContent
          headerMessage={`Realmente deseja ${action} a venda do dia ${moment(
            sale.date
          ).format("DD/MM/YYYY")} feita por ${
            sale.user?.name
          } no valor de ${formattedCurrencyWithSymbol(sale.saleAmount)}?`}
          inputProps={{
            control: control,
            errors: errors,
            name: "feedback",
              ...
              <FormControl sx={{ minWidth: 280 }}>
                ...
                <Select
                  ...
                  onChange={e => {
                    ...
                    setFilteredData(
                      filterSalesByDate(e.target.value as DateRangeOptions)
                    )
                  }}
                >
