...
const ClientVersusSale = () => {
  ...
  const searchSale = async (data: any) => {
    ...
    const toastId = toast.loading("Buscando venda...", { position: "top-center" });
    try {
      const sale = await getSaleBySaleNumber(Number(data.saleNumber));
      if (sale?.length === 0) return toast.error("Venda não encontrada! Verifique se o código inserido está correto", {
        id: toastId,
        position: "top-center"
      });
      setSale(sale[0] as Sale);
      toast.success("Venda encontrada!", { id: toastId, position: "top-center" });
    } catch (error) {
      toast.error("Venda não encontrada! Verifique se o código inserido está correto", {
        id: toastId,
        position: "top-center"
      });
    } finally {
  ...
  const searchClient = async (data: any) => {
    ...
    const toastId = toast.loading("Buscando cliente...", { position: "top-center" });
    try {
      const client = await getClientByClientNumber(Number(data.clientNumber));
      if (client?.length === 0) return toast.error("Cliente não encontrado! Verifique se o código inserido está correto", {
        id: toastId,
        position: "top-center"
      });
      ...
      toast.error("Cliente não encontrado! Verifique se o código inserido está correto", {
        id: toastId,
        position: "top-center"
      });
    } finally {
  ...
  const updateSaleClient = async () => {
    const toastId = toast.loading("Atualizando venda...", { position: "top-center" });
    try {
      await updateSale.mutateAsync({ saleID: sale?._id as string, clientID: client?._id as string });
      toast.success("Venda atualizada!", { id: toastId, position: "top-center" });
      clientReset();
      saleReset();
      setClient(null);
      setSale(null);
    } catch (e) {
      toast.error("Erro ao atualizar venda!", { id: toastId, position: "top-center" });
    }
  };
  ...
  return (
    <Grid container spacing={6}>
      <Grid item xs={12}>
        <Typography variant="h5" sx={{ display: "flex", alignItems: "center" }} marginBottom={3} gap={2} px={5}>
          <SupervisedUserCircleIcon sx={{ color: "primary.main", fontSize: 30 }} /> Cliente x Venda
        </Typography>
        <Card>
          <CardContent>
            <Grid container spacing={6}>
              ...
              <Grid item xs={12} sx={{ display: "flex", gap: 3, justifyContent: "center" }}>
                <form onSubmit={saleHandleSubmit(searchSale)}>
                  <Typography sx={{ display: "flex", gap: 3 }}>
                    <TextInputControlled
                      ...
                      errors={saleErrors}
                    />
                    <LoadingButton loading={isLoadingSale} type={"submit"} variant={"outlined"}>
                      Pesquisar
                    </LoadingButton>
                  </Typography>
                </form>
              ...
              <Grid item xs={12} sx={{ display: "flex", gap: 3, justifyContent: "center" }}>
                {
                  sale &&
                  <SaleCardList newSaleMockup={sale} />
                }
              </Grid>
              <Grid item xs={12}>
                <Typography variant="h6" sx={{ display: "flex", alignItems: "center", justifyContent: "center" }}
                ...
                <form onSubmit={clientHandleSubmit(searchClient)}>
                  <Typography sx={{ display: "flex", gap: 3 }}>
                    <TextInputControlled
                      ...
                      errors={clientErrors}
                    />
                    <LoadingButton loading={isLoadingClient} type={"submit"} variant={"outlined"}>
                      Pesquisar
                    </LoadingButton>
                  </Typography>
                </form>
              ...
              <Grid item xs={12} sx={{ display: "flex", gap: 3, justifyContent: "center" }}>
                {
                  client &&
                  <List dense subheader={<ListSubheader>Informações do Cliente</ListSubheader>}>
                    ...
                    <ListItem disablePadding>
                      <ListItemButton>
                        ...
                        <ListItemText
                          primary={`Criado em: ${moment(client._createdAt).format("DD/MM/YYYY") || "01/01/2021"}`}
                        />
                      </ListItemButton>
                    </ListItem>
                    <ListItem disablePadding>
                      <ListItemButton>
                        ...
                        <ListItemText
                          primary={`Loja: ${client.store.name}`}
                        />
                      </ListItemButton>
                    </ListItem>
                    <ListItem disablePadding>
              ...
              <Grid item xs={12} sx={{ display: "flex", gap: 3, justifyContent: "center" }}>
                <LoadingButton onClick={updateSaleClient} loading={isLoading} disabled={!client || !sale}
                               variant={"contained"}>
                  <SaveIcon sx={{ mr: 1 }} />
                  Salvar
                </LoadingButton>
