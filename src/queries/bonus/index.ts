...
export const useGetBonusByReferenceIdQuery = (referenceId: string, options?: Object) => {
  return useQuery(["bonus", 'referenceId', `${referenceId}`], () => bonusApi.getBonusByReferenceId(referenceId), {
    // 1hr stale time
    staleTime: 1000 * 60 * 60,
    // 6hr cache time
    cacheTime: 1000 * 60 * 60 * 6,
...
export const useUpdateBonusFieldsQuery = (queryClient: any) => {
  return useMutation(({id, bonus}:{id: string, bonus: Partial<IBonus>}) => bonusApi.updateBonusFields(id, bonus),
    {
      onSuccess: (updatedBonus) => {
        queryClient.setQueryData(["bonus", "all"], (old: any) => {
          if (old) {
