...
export const salePerPaymentMethodStats = (
  sale: Sale[]
): salePerPaymentMethod[] | [] => {
  ...
  sale.forEach((sale: Sale) => {
    ...
    const paymentMethodAlreadyInResult = result.find(
      (result: salePerPaymentMethod) =>
        result.paymentMethod?._id === sale.paymentMethod?._id
    )
    ...
    if (paymentMethodAlreadyInResult) {
      ...
      paymentMethodAlreadyInResult.average =
        paymentMethodAlreadyInResult.amount / paymentMethodAlreadyInResult.count
    }
...
export const costsPerPaymentMethodStats = (
  sale: Sale[]
): salePerPaymentMethod[] | [] => {
  const result: salePerPaymentMethod[] = []
...
export const salePerVendorStats = (sale: Sale[]): salePerVendor[] | [] => {
  ...
  sale.forEach((sale: Sale) => {
    ...
    const vendorAlreadyInResult = result.find(
      (result: salePerVendor) => result.vendor?._id === sale.vendor?._id
    )
    ...
    if (vendorAlreadyInResult) {
      ...
      vendorAlreadyInResult.average =
        vendorAlreadyInResult.amount / vendorAlreadyInResult.count
    }
